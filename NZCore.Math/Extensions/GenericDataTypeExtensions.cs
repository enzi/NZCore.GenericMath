...
namespace NZCore
{
    public static class GenericDataTypeExtensions
    {
        ...
        public static void AddGenericValue<T>(this ref DynamicBuffer<T> buffer, GenericDataType dataType, string value)
            where T : unmanaged
        {
            ...
            switch (dataType)
            {
                ...
                case GenericDataType.Byte:
                {
                    buffer.AddToByteBuffer(byte.Parse(value));
                    break;
                }
                case GenericDataType.Short:
                {
                    buffer.AddToByteBuffer(short.Parse(value));
                    break;
                }
                case GenericDataType.UShort:
                {
                    buffer.AddToByteBuffer(ushort.Parse(value));
                    break;
                }
                case GenericDataType.Half:
                {
                ...
                case GenericDataType.Float:
                {
                    buffer.AddToByteBuffer(float.Parse(value));
                    //Debug.Log($"set {statType} to {value}");
                    break;
                }
                case GenericDataType.Int:
                {
                    buffer.AddToByteBuffer(int.Parse(value));
                    break;
                }
                case GenericDataType.UInt:
                {
                    buffer.AddToByteBuffer(uint.Parse(value));
                    break;
                }
                case GenericDataType.Double:
                {
                    buffer.AddToByteBuffer(double.Parse(value));
                    //Debug.Log($"set {statType} to {value}");
                    break;
                }
                case GenericDataType.Long:
                {
                    buffer.AddToByteBuffer(long.Parse(value));
                    break;
                }
                case GenericDataType.ULong:
                {
                    buffer.AddToByteBuffer(ulong.Parse(value));
                    break;
                }
                case GenericDataType.BigDouble:
                {
                    buffer.AddToByteBuffer(BigDouble.Parse(value));
                    break;
                }
                case GenericDataType.None:
